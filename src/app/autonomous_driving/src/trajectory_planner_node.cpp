...
void TrajectoryNode::Run() {
    ...
    if(drive_mode == MERGE)
    {
        ...
        Point back_point = BackTargetPoint(static_position, 15, driving_way, vehicle_state);
...
Point TrajectoryNode::BackTargetPoint(const geometry_msgs::msg::Point& static_position, double back_distance, const ad_msgs::msg::PolyfitLaneData& driving_way, const ad_msgs::msg::VehicleState& vehicle_state) {
    
    double yaw_rate = vehicle_state.yaw_rate;
    // Calculate the relative position in the global frame
    double dx = static_position.x - i_vehicle_state_.x;
    double dy = static_position.y - i_vehicle_state_.y;
